...
import { GetUsers } from '../../store/Actions/role';
import {  GetAllUsers, SaveIdForUserManagement, ClearState,GetActiveRoles } from '../../store/Actions/userManagement';
...
class SearchUser extends Component {
  ...
  addMember() {
    ...
    //   genericArea: false,
    // })
    // this.props.onGetActiveRoles().then(
    //   (res) => {
  ...
  editRow(row) {
    ...
    this.props.onSaveIdForUserManagement(data).then(
      (res) => {
        this.props.history.push('/UserManagement/EditUser');
      }
    );
  ...
  render() {
    ...
    function avatarFormatter(params, row) {
      return <Useravatar
        Useravatarname={row.firstName}
        rowData={row} />
    }

    function anchorFormatter(cell, row) {
      return <Anchorcomponent
        anchotDisplayName={row.firstName}
        rowData={row}
    ...
    if (this.props.error == null || this.props.error == '' && this.props.roles != null) {
      ...
      dataTable =
        <BootstrapTable
          data={this.props.usersManagementData}
          pagination={true}
          search={true}
          ...
          striped
          hover>
          <TableHeaderColumn width="50" isKey dataField='id'>Login ID</TableHeaderColumn>
          <TableHeaderColumn width="50" dataFormat={avatarFormatter.bind(this)} dataField='firstName'></TableHeaderColumn>
          <TableHeaderColumn dataSort={true} dataFormat={anchorFormatter.bind(this)} dataField='firstName'>First Name</TableHeaderColumn>
...
const mapStateToProps = state => {
  return {
    ...
    usersManagementData: state.reducerUserManagement.usersManagementData
  }
}
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    onGetAllUsers: (data) => dispatch(GetAllUsers(data)),
    onSaveIdForUserManagement: (data) => dispatch(SaveIdForUserManagement(data)),
